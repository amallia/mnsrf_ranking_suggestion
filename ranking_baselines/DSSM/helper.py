...
import os, glob, pickle, math, time, torch, numpy, re, string
...
def sequence_to_tensors(sequence, dictionary):
    ...
    for i in range(len(sequence)):
        if sequence[i] in dictionary.word2idx:
...
def batch_to_tensor(batch, dictionary):
    ...
    for i in range(len(batch)):
        query_tensor[i] = sequence_to_tensors(batch[i].query_terms, dictionary)
        # shuffle the list of relevant documents
        numpy.random.shuffle(batch[i].rel_docs)
        for j in range(len(batch[i].rel_docs)):
            query_clicks[i, j] = sequence_to_tensors(batch[i].rel_docs[j].title, dictionary)
