...
def batch_to_tensor(batch, dictionary, max_q_len, max_d_len):
    ...
    for i in range(len(batch)):
        query_tensor[i] = sequence_to_tensors(batch[i].letter_n_grams, max_q_len, dictionary)
        # shuffle the list of relevant documents
        numpy.random.shuffle(batch[i].rel_docs)
        for j in range(len(batch[i].rel_docs)):
            query_clicks[i, j] = sequence_to_tensors(batch[i].rel_docs[j].letter_n_grams, max_d_len, dictionary)
