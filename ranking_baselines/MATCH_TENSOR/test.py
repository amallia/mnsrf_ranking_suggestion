...
def test_ranking(model, test_batches):
    ...
    for batch_no in range(1, num_batches + 1):
        queries, query_len, docs, doc_len, click_labels = helper.batch_to_tensor(test_batches[batch_no - 1],
        ...
        score = model(queries, query_len, docs, doc_len)
        numpy_score, numpy_labels = score.data.cpu().numpy(), click_labels.data.cpu().numpy()
        _map += mean_average_precision(numpy_score, numpy_labels)
        mrr += MRR(numpy_score, numpy_labels)
        ndcg_1 += NDCG(numpy_score, numpy_labels, 1)
        ndcg_3 += NDCG(numpy_score, numpy_labels, 3)
        ndcg_5 += NDCG(numpy_score, numpy_labels, 5)
        ndcg_10 += NDCG(numpy_score, numpy_labels, 10)
