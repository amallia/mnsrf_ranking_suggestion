...
class Train:
    ...
    def train_epochs(self, train_corpus, dev_corpus, start_epoch, n_epochs):
        ...
        for epoch in range(start_epoch, start_epoch + n_epochs):
            if not self.stop:
                ...
                helper.save_plot(self.dev_losses, self.config.save_path, 'dev_loss_plot', epoch + 1)
            else:
                break
    ...
    def train(self, train_corpus):
        ...
        for batch_no in range(1, num_batches + 1):
            ...
            session_queries, session_query_length = helper.session_to_tensor(train_batches[batch_no - 1],
                                                                             self.dictionary)
            if self.config.cuda:
    ...
    def validate(self, dev_corpus):
        ...
        for batch_no in range(1, num_batches + 1):
            session_queries, session_query_length = helper.session_to_tensor(dev_batches[batch_no - 1], self.dictionary,
                                                                             True)
            if self.config.cuda:
                session_queries = session_queries.cuda()
                session_query_length = session_query_length.cuda()
