...
def suggest_next_query(model, session_queries, session_query_length, next_query_tensor):
    ...
    sess_query_hidden = model.session_encoder.init_weights(encoded_queries.size(0))
    hidden_states = []
    ...
    for idx in range(encoded_queries.size(1)):
        ...
        sess_q_out, sess_q_hidden = model.session_encoder(encoded_queries[:, idx, :].unsqueeze(1), sess_query_hidden)
...
if __name__ == "__main__":
    ...
    for sess_len, sessions in test_corpus.data.items():
        for session in sessions:
            ...
            scores = []
            session_queries, session_query_length = helper.session_to_tensor([session], dictionary)
            for cand in cands:
