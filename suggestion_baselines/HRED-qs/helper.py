...
def session_to_tensor(sessions, dictionary, iseval=False):
    ...
    for i in range(len(sessions)):
        for j in range(len(sessions[i].queries)):
            session_queries[i, j] = sequence_to_tensors(sessions[i].queries[j].query_terms, max_query_length, dictionary)
            session_query_length[i, j] = len(sessions[i].queries[j].query_terms)

    if iseval:
        return Variable(session_queries, volatile=True), Variable(session_query_length, volatile=True)
    else:
