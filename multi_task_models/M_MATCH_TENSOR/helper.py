...
def session_to_tensor(sessions, dictionary, iseval=False):
    ...
    rel_docs = torch.LongTensor(len(sessions), len(sessions[0]), len(sessions[0][0].rel_docs),
                                max_document_length)
    # batch_size x session_length x num_rel_docs_per_query
    rel_docs_length = torch.LongTensor(len(sessions), len(sessions[0]), len(sessions[0][0].rel_docs))
    # batch_size x session_length x num_rel_docs_per_query
    doc_labels = torch.FloatTensor(len(sessions), len(sessions[0]), len(sessions[0][0].rel_docs))
    for i in range(len(sessions)):
        for j in range(len(sessions[i])):
            session_queries[i, j] = sequence_to_tensors(sessions[i][j].text, max_query_length, dictionary)
            session_query_length[i, j] = len(sessions[i][j].text)
            # shuffle the list of relevant documents
            np.random.shuffle(sessions[i][j].rel_docs)
            for k in range(len(sessions[i][j].rel_docs)):
                rel_docs[i, j, k] = sequence_to_tensors(sessions[i][j].rel_docs[k].text, max_document_length,
                                                        dictionary)
                rel_docs_length[i, j, k] = len(sessions[i][j].rel_docs[k].text)
                doc_labels[i, j, k] = 1 if sessions[i][j].rel_docs[k].is_clicked else 0
